...
c_landing = pd.read_csv(r"...\resources\scripts\c_landing3000_script_test.csv",  encoding='utf-8-sig', sep='\s*,\s*', engine='python')
...
##Just show one flight landing



##Just show one flight landing
#single_landing=df_ld.get_group(('02b3425f-3d7f-48d9-97ab-aaae212a494c'))


param_1_ = sys.argv[1]
single_landing=df_ld.get_group((param_1_))

for index, row in single_landing.iterrows():
    if(row['detection']==True):
        gmap.marker(row['lat'], row['lon'], "#00FF00", title="Normal")
    else:
        gmap.marker(row['lat'], row['lon'], "#FF0000", title="Abnormal")
gmap.plot(single_landing['lat'], single_landing['lon'],
           '#FF0000', edge_width = 2.5)

#single_landing=df_ld.get_group(('000de98b-62bf-47e7-b19a-b63a9920dba8'))
#for index, row in single_landing.iterrows():
#    if(row['detection']==True):
#        gmap.marker(row['lat'], row['lon'], "#00FF00", title="Normal")
#    else:
...
#single_landing=df_ld.get_group(('02cb4a06-1936-4292-af97-cb54b2cded57'))
#for index, row in single_landing.iterrows():
#    if(row['detection']==True):
#        gmap.marker(row['lat'], row['lon'], "#00FF00", title="Normal")
#    else:
...
#         gmap.plot(row['lat'], row['lon'], '#00FF00',  size = 5 )
#     else:
#         gmap.plot( row['lat'], row['lon'], '#00FF00',  size = 5 )
#     print(row['lat'], row['lon'])
#     single_landing= single_landing.filter(['lat', 'lon'])



#single_landing=df_ld.get_group(('001451ef-cdfb-445f-a234-deea69d7aca0'))
#for index, row in single_landing.iterrows():
#    if(row['detection']==True):
#        gmap.marker(row['lat'], row['lon'], "#00FF00", title="Normal")
#    else:
...
# Plot method Draw a line in
# between given coordinates
# gmap3.plot(single_landing['lat'], single_landing['lon'],
#            'cornflowerblue', edge_width = 2.5)
gmap.draw("runway_landing1.html")
#gmap3.draw("map"+str(id)+".html")
